...
for line in f:#for each line in the file, split it up by |
    ...
    for i in range(len(entries)-2):#populate the dictionary with names as keys
            # attributes_list= [entries[1], entries[2])#will need a solution if attributes added later
            #     for elem in attributes_list:
            #         key = "cost"
            #         attributes_dict[key] = elem[0]
            #value = (entries[-1:1])#makes this flexible by adding everything from the end to the second entry
            key= entries[0]
            #value = []
            ...
            #print("this is the current value", melon_dict[key])
#print("test, line 26", melon_dict)
